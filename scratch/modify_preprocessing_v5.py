...
def modify_preprocessing_v5():
    ...
    cell_9_source = [
        ...
        "\n",
        "# --- 2. Apply Text Cleaning (PURE TEXT ONLY for XAI/LIME/SHAP mapping) ---\n",
        "df['review_clean'] = df[TEXT_COL].apply(clean_text)\n",
        "\n",
        "# Hapus baris kosong atau terlalu pendek\n",
        "df = df.dropna(subset=['review_clean'])\n",
        "df = df[df['review_clean'].str.strip().str.len() > 3].copy()\n",
        "len_clean = len(df)\n",
        "\n",
        "# --- 3. Deduplicate on Cleaned Text (keep the first occurrence to avoid leakage) ---\n",
        "df = df.drop_duplicates(subset=['review_clean'], keep='first').copy()\n",
        "len_dedup = len(df)\n",
        ...
        "        \"2. Setelah Filter Teks Pendek (Cleaned)\",\n",
        "        \"3. Setelah Deduplikasi Teks Bersih (Deduplicated)\"\n",
        "    ],\n",
        "    \"Jumlah Baris\": [len_raw, len_clean, len_dedup],\n",
        "    \"Baris Dibuang\": [0, len_raw - len_clean, len_clean - len_dedup]\n",
        "}\n",
